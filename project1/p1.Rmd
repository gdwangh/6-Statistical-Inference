...
qqnorm(df$average, col="blue")
qqline(df$average,col="red")
...
it look like normal distribution and qqplot show the data sample is near the a “theoretical” line which is normal by default 
...
lambda<-0.2
set.seed(2)
nvals<-seq(40,1000,by=10)
coverages<-sapply(nvals, function(n) {
  xhats<-matrix(rexp(nosim*n, lambda), ncol=n)
  miu<-apply(xhats, 1, mean)
  S<-apply(xhats, 1, sd)
  ll <- miu - 1.96*S/sqrt(n)
...
g <- g + geom_line(data=data.frame(nvals,coverages), aes(x=nvals,y=coverages),size=1)
